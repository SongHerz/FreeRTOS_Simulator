...
/* For register signals that minic hardware interrupts.
 * This is intend for virtual device initialization only.
 * And CAN NOT BE USED IN A TASK.
 */
/* Return pdTRUE on success, pdFALSE on error */
extern portBASE_TYPE xPortRegisterInterruptSignal( int signum );
#define	portREGISTER_INTERRUPT_SIGNAL( SIGNUM )	xPortRegisterInterruptSignal( SIGNUM )
/*-----------------------------------------------------------*/
...
extern portBASE_TYPE xPortInterruptEnabled( void );
...
extern void vPortAddInterruptBottomHalf( void (*bh)(void*), void *param );

#define portADD_INTERRUPT_BOTTOM_HALF( BH, PARAM )	vPortAddInterruptBottomHalf( (BH), (PARAM) )
/*-----------------------------------------------------------*/
...
/* AIO read signal.
 * SIGRTMIN and ( SIGRTMIN + 1 ) are used by pthread in glibc-2.7
 */
