...
#if defined(__GCC_POSIX__) && !defined(NDEBUG)
...
void vTaskSuspendAll( void )
{
	...
	portBASE_TYPE. */
#ifdef	__GCC_POSIX__
	/* The POSIX simlator may run on multi-core CPU, and
	 * the operating system may schedule threads, so a task may
	 * have been switched by FreeRTOS architecture independent
	 * part, but not suspended by architecture dependent part.
	 * If a threads is scheduled like this:
	 * 1. task switched by FreeRTOS independent code.
	 * 2. task thread is scheduled by operating system
	 *    and uxShcedulerSuspended is altered.
	 * 3. task thread is suspended by archtecture dependent code.
	 *
	 * And, if the resumed task thread is not the same as suspended,
	 * the resumed thread, will see wrong uxSchedulerSuspended, and
	 * will not switch out.
	 *
	 * I didn't think carefully about SCHED_FIFO for pthread.
	 * But that scheduler policy requires privilege, I don't
	 * like that, since it may be dangerous for the operation system.
	 */
	portENTER_CRITICAL();
#endif
